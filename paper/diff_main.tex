...
%DIF ADD main.tex       Mon Sep  1 15:51:46 2025
...
the average rate will be increased, causing the walk length for the entire next batch to decrease.
...
The average \DIFdelbegin \DIFdel{chain length }\DIFdelend \DIFaddbegin \DIFadd{number of likelihood evaluations }\DIFaddend for the \texttt{blackjax-ns}
sampler was \DIFdelbegin \DIFdel{1376}\DIFdelend \DIFaddbegin {} \DIFadd{million}\DIFaddend , consistent with the average of \DIFdelbegin \DIFdel{1337 }\DIFdelend \DIFaddbegin {} \DIFadd{million }\DIFaddend for the
...
in an average of \DIFdelbegin \DIFdel{0.4 }\DIFdelend \DIFaddbegin {} \DIFaddend hours per injection, with individual run
times ranging from \DIFdelbegin \DIFdel{9 to 56 }\DIFdelend \DIFaddbegin {} \DIFadd{to }{} \DIFaddend minutes.
...
$\times$\DIFdelbegin \DIFdel{81.2}\DIFdelend \DIFaddbegin {}\DIFaddend . As in 
...
corresponds to an average cost-reduction factor of $\times$\DIFdelbegin \DIFdel{5.1 }\DIFdelend \DIFaddbegin {} \DIFaddend for the
...
}\DIFaddend a batch size of \texttt{num\_delete} = 700 \DIFaddbegin \DIFadd{and }{\texttt{\DIFadd{naccept}} \DIFadd{= 120}}\DIFaddend , completed in \DIFdelbegin \DIFdel{0.45 }\DIFdelend \DIFaddbegin \DIFadd{0.82 }\DIFaddend hours.
This corresponds to a further speedup of $\times$\DIFdelbegin \DIFdel{23.2 }\DIFdelend \DIFaddbegin \DIFadd{12.7 }\DIFaddend over the
sequential-GPU run.
...
overall speedup (a total of $\times$\DIFdelbegin \DIFdel{67.6 }\DIFdelend \DIFaddbegin \DIFadd{37.1 }\DIFaddend in this case) originates from the
...
}\texttt{\DIFdel{naccept}} %DIFAUXCMD
\DIFdel{parameter from 120 to 60 to match the }\texttt{\DIFdel{bilby}} %DIFAUXCMD
...
translates sub-optimally to the batched GPU paradigm. \DIFdelbegin \DIFdel{This necessitates
manual adjustment of the sampler settings for different problems,
as the kernel }\DIFdelend \DIFaddbegin \DIFadd{The design of the kernel,
implemented to match }\texttt{\DIFadd{bilby}}\DIFadd{'s implementation as closely as possible,
}\DIFaddend is not natively optimised for this
...
more robust and efficient performance without \DIFdelbegin \DIFdel{problem-specific tuning}\DIFdelend \DIFaddbegin \DIFadd{the same efficiency problems}\DIFaddend .
