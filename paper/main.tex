...
%may be useful here to frame this as a benchmark for future work here, e.g.:

%The original work of \cite{yallup2025nested} proposed a Hit-and-Run Slice Sampling~\citep{Neal2003_slice} for this task, alongside providing a general vectorized framework we leverage in this work. Establishing this baseline demonstrates the flexibility of the \texttt{blackjax} nested sampling framework, and gives scope for future work to explore other parallel inner sampling methods.
...
limit.
% worth commenting here that delay is functionally similar to just averaging over a batch anyway?
% should discuss here the setting of max_mcmc steps, we want this section to list all relevant hyperparameters.

%may also be worth here discussing breifly what the parameters do, e.g. longer walk length -> more robust decorrelation, larger population -> better resolution. Although these are standard to bilby, it may be useful for broader audiences
...
% in practice presumably pbilby also does this, just to the tune of n_cores available? At least polychord parallelisation is also sawtoothing
% may be worth clarifying that this sawtoothing is common to all parallel nested sampling, and it's only the massively parallel that makes it visible
...
%would be really useful here or in appendix to have a log-log plot of likelihood evaluation time vs n_particles
% if this shows a linear trend as I gather we are seeing somewhere on [1,1000] particles, then it is useful to frame the best speedup we can expect on a single card, and use this as ammo to point to either multi-GPU, or the compression methods mentioned above
...
% cost saving is a nice touch, but may be worth motivating that all future compute resource R&D + money is going to GPUs, so we would expect to only see the cost comparison get more favourable (even if runtime stays the same)
% additionally we can motivate an additional utility of this work. 
% The \texttt{jax}~\cite{jax2018github} ecosystem is very well suited to distribution over multiple devices and architectures. It has been closely developed alongside the TPU architecture~\cite{jouppi2023tpu}, giving a clear avenue to mix this work with such frontier low-cost hardware.
...
The performance gains from a GPU sampling framework arises from the ability to exploit massive parallelism. In particular, we seek to exploit parallelism across multiple short Markov chains, as this is the central construction of NS. The potential bottlenecks for parallelism arise from the NS algorithms ability to effectively synchronise the chains, and (if this condition is met) the ability to parallelise the likelihood evaluation itself across these chains. Developing algorithms that can effectively achieve the former goal is part of the ongoing programme of research this work establishes on a GPU GW problem. Regardless of effective parallelism across chains, the algorithm in this work is bottlenecked by the ability to parallelise the likelihood evaluation itself. We demonstrate the scaling of the likelihood evaluation time with the number of samples in Figure~\ref{fig:scaling_l4}. The emerging pattern of polynomial runtime scaling with the number of samples, particularly for the \emph{Full} bin likelihood we investigate primarily in this work, indicates that the parallel throughput of the GPU is being saturated. This bottleneck can be mitigated by either; using a compressed likelihood (\emph{Heterodyning} being an example of this shown in the figure), expanding processing over multiple GPUs, or -- if possible -- rewriting the likelihood to be more parallelism friendly.

\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{figures/scaling_l4.pdf}
    \caption{Scaling of likelihood wallclock time on an NVIDIA L4 GPU. A typical representative \texttt{jimgw} likelihood for H1 and L1 detectors on a 4 second window using the \texttt{IMRPhenomD} waveform model from \texttt{ripple} is used. Calls of likelihood vectorized over $N$ samples are made, and the wall time averaged over 50 calls is reported. Comparison is made between the \emph{Full} likelihood using all frequency bins, and a \emph{Heterodyned} likelihood compressing to 300 frequency bins.}
