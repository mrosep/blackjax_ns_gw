...
detections of the Laser Interferometer Gravitational-Wave Observatory (LIGO),
...
substantial, orders-of-magnitude speedups are
achievable for GW parameter estimation. While these MCMC-based approaches excel at rapidly generating samples
from the posterior, they do not directly compute the
...
vectorization, it introduces some important differences. In the sequential CPU
...
evaluations compared to the sequential counterpart. We discuss this further in Sections~\ref{sec:sampler_config} 
and~\ref{sec:results}
...
\texttt{bilby} parameters such as \texttt{naccept} may no longer be optimal. For
...
cost-saving factor of approximately $\times$3.7 for the GPU-based analysis.
...
and accepted steps per iteration to \texttt{bilby} (see Figure~\ref{fig:chain_length_comparison}), enabling a fairer and more direct comparison
...
This corresponds to a further speedup of $\times$23.2 over the
sequential-GPU run.
...
overall speedup (a total of $\times$67.6 in this case) originates from the
...
better suited to GPUs and may offer
...
\textcolor{red}{TODO: Add data availability statement.}
