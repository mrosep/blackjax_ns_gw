#!/usr/bin/env python3
"""
BlackJAX Nested Sampling for Gravitational Wave Parameter Estimation
Converted from bilbykernel_unitcube_4s.ipynb

This script performs GPU-accelerated nested sampling for gravitational wave 
parameter estimation using BlackJAX with a phase-marginalized likelihood.
"""
...
def main():
    print("Starting BlackJAX Nested Sampling for Gravitational Wave Inference")
    
    # Initialize waveform
    waveform = RippleIMRPhenomD(f_ref=50)

    # Noise curve paths - now using local files
    asd_paths = {
    ...
    # Load detector data as JAX arrays - now using local files
    detector_data = {
    ...
    print(f"Initial termination condition: {terminate(state)}")
    ...
    output_filename = "bilbykernel_unitcube_4s_results.csv"
    ...
    if hasattr(final_state, 'inner_kernel_info') and final_state.inner_kernel_info is not None:
        ...
        f.tight_layout()
        f.savefig("adaptive_chain_diagnostics.pdf")
        print("Saved adaptation diagnostics to adaptive_chain_diagnostics.pdf")
        ...
        print(f"Mean accepted steps per point: {np.mean(n_accept_per_point_history):.1f} (target: 60)")
    ...
    if hasattr(final_state, 'inner_kernel_info') and final_state.inner_kernel_info is not None:
        ...
        if hasattr(info, 'n_accept') and hasattr(info, 'n_steps'):
            ...
            axes[0].set_title("Accepted Steps per NS Iteration")
            ...
            axes[1].set_title("Steps per NS Iteration")
            ...
            axes[2].set_title("Acceptance Rate per NS Iteration")
            ...
            f.tight_layout()
            f.savefig("performance_diagnostics.pdf")
            print("Saved performance diagnostics to performance_diagnostics.pdf")
