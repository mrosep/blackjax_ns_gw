"""JAX-native implementation of Bilby adaptive 'acceptance-walk' kernel with wraparound support.
...
This ADAPTIVE version with WRAPAROUND support replicates Bilby's exact behavior where 
the chain length (num_walks) adapts each batch to target a specific number of accepted 
steps (typically 60), with configurable stepper functions for periodic boundary handling.
"""
...
def default_de_stepper(position, direction, step_size):
    """Default stepper for DE proposals: position + step_size * direction"""
    return jax.tree.map(lambda pos, d: pos + step_size * d, position, direction)


def create_wrapped_stepper(mask_tree, period_tree):
    """Factory for wrapped stepper handling periodic parameters.
    
    Parameters
    ----------
    mask_tree : ArrayTree
        Boolean mask indicating which parameters are periodic  
    period_tree : ArrayTree
        Period values for periodic parameters
        
    Returns
    -------
    stepper_fn : callable
        Stepper with signature (position, direction, step_size) -> position
    """
...
def de_rwalk_one_step_physical(
    ...
    Uses configurable stepper function for boundary handling.
    """
    ...
    # PyTree-aware way to get number of live particles
    ...
    # Differential evolution vector - PyTree-aware indexing and subtraction
    ...
    # Physical space proposal using configurable stepper
    ...
    # Conditional state update - PyTree-aware conditional selection
...
def update_bilby_walks_fn(
    ...
    # PyTree-aware way to get number of live particles (works for both arrays and dicts)
    ...
    # PyTree-aware way to get total dimensionality
...
def bilby_adaptive_de_sampler(
    ...
    max_mcmc: int = 5000,
    num_delete: int = 1,
    stepper_fn: callable = default_de_stepper,
) -> SamplingAlgorithm:
    """Creates a Bilby-style adaptive DE sampler with configurable stepper function.
    ...
    - Configurable stepper function for boundary handling (default, wrapped, etc.)
    ...
    stepper_fn
        Stepper function with signature (position, direction, step_size) -> position.
        Defaults to standard Euclidean stepper. Use create_wrapped_stepper() for 
        periodic parameters.
        
    Returns
    -------
    SamplingAlgorithm
        BlackJAX sampling algorithm with adaptive chain length and configurable stepper.
    """
    ...
    # Use partial to bind stepper to the dynamic walker
    kernel_with_stepper = partial(de_rwalk_dynamic, stepper_fn=stepper_fn)
