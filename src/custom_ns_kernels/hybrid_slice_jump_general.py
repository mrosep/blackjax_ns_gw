...
def update_hybrid_params_fn(ns_state, *args, **kwargs):
    ...
    **kwargs
    ...
    """
    cov_params = compute_covariance_from_particles(ns_state, None)
    return {
        "live_points": ns_state.particles,
        **cov_params,  # Adds 'cov' in correct PyTree format
    }
...
def hybrid_one_step(
    ...
    def de_jump_fn(de_key):
        ...
        # DE proposal: current position + unnormalized_direction (gamma=1.0)
...
def hybrid_slice_jump_sampler(
    ...
    one_step_fn = partial(hybrid_one_step, p_slice=p_slice, direction_fn=direction_fn)
